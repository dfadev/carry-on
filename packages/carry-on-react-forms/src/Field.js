...
import { MemoizedFieldContextProvider } from "./FieldContext";
  ...
  return (
    <FormState id={path} {...rest}>
      {(form, store, prefix) => {
        ...
        let prefixedPath;
        if (path === undefined) prefixedPath = prefix || "field";
        else if (prefix !== undefined) prefixedPath = `${prefix}.${path}`;
        else prefixedPath = path;
        ...
        return (
          <MemoizedFieldContextProvider prefix={nextPrefix}>
            {children(
              {
